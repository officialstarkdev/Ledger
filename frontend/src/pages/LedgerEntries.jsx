                                    ...
                                    <tbody>
                                        {viewEntry.entries.map((e, i) => (
                                            <tr key={i} className="border-b border-dark-100 dark:border-dark-800/50">
                                                <td className="py-2 text-dark-600 dark:text-dark-400">{new Date(e.date).toLocaleDateString()}</td>
                                                <td className="py-2 text-dark-700 dark:text-dark-300">{e.description}</td>
                                                <td className="py-2 text-right text-danger-500">{e.debit > 0 ? `$${e.debit.toFixed(2)}` : '-'}</td>
                                                <td className="py-2 text-right text-success-500">{e.credit > 0 ? `$${e.credit.toFixed(2)}` : '-'}</td>
                                                <td className="py-2 text-right font-medium text-dark-700 dark:text-dark-300">${e.balance.toFixed(2)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
