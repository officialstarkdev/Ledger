...
const Transactions = () => {
    ...
    const handleExportCSV = () => {
        const headers = 'Date,Type,Category,Description,Amount,Profit,Balance\n';
        const rows = transactions.map((t) =>
            `${new Date(t.date).toLocaleDateString()},${t.type},${t.category},"${t.description}",${t.amount},${t.profit || 0},${t.balance}`
        ).join('\n');
    ...
    const handleExportPDF = async () => {
        ...
        autoTable(doc, {
            startY: 36,
            head: [['Date', 'Type', 'Category', 'Description', 'Amount', 'Profit', 'Balance']],
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-dark-200 dark:border-dark-800">
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wider">Date</th>
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wider">Type</th>
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wider">Category</th>
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-dark-500 uppercase tracking-wider">Description</th>
                                        <th className="px-5 py-3 text-right text-xs font-semibold text-dark-500 uppercase tracking-wider">Amount</th>
                                        <th className="px-5 py-3 text-right text-xs font-semibold text-dark-500 uppercase tracking-wider">Profit</th>
                                        <th className="px-5 py-3 text-right text-xs font-semibold text-dark-500 uppercase tracking-wider">Balance</th>
                                        <th className="px-5 py-3 text-right text-xs font-semibold text-dark-500 uppercase tracking-wider">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {transactions.map((tx, i) => (
                                        ...
                                        >
                                            <td className="px-5 py-3.5 text-sm text-dark-700 dark:text-dark-300">{new Date(tx.date).toLocaleDateString()}</td>
                                            <td className="px-5 py-3.5">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${tx.type === 'credit' ? 'bg-success-500/10 text-success-500' : 'bg-danger-500/10 text-danger-500'
                                                    }`}>
                                                    {tx.type}
                                                </span>
                                            </td>
                                            <td className="px-5 py-3.5 text-sm text-dark-700 dark:text-dark-300">{tx.category}</td>
                                            <td className="px-5 py-3.5 text-sm text-dark-500 dark:text-dark-400 max-w-[200px] truncate">{tx.description}</td>
                                            <td className={`px-5 py-3.5 text-sm text-right font-semibold ${tx.type === 'credit' ? 'text-success-500' : 'text-danger-500'}`}>
                                                {tx.type === 'credit' ? '+' : '-'}${tx.amount.toFixed(2)}
                                            </td>
                                            <td className="px-5 py-3.5 text-sm text-right font-medium text-dark-700 dark:text-dark-300">
                                                ${(tx.profit || 0).toFixed(2)}
                                            </td>
                                            <td className="px-5 py-3.5 text-sm text-right font-medium text-dark-700 dark:text-dark-300">${tx.balance.toFixed(2)}</td>
